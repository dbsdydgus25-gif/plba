...
// 플로우: email → (링크 클릭 → callback → ?step=password) → password → bizinfo → bizreg → store → name → phone → sms → done
type Step = "email" | "waiting" | "password" | "bizinfo" | "bizreg" | "store" | "name" | "phone" | "sms" | "done";

const STEPS: Step[] = ["email", "waiting", "password", "bizinfo", "bizreg", "store", "name", "phone", "sms", "done"];
// 진행바는 password 이후부터 (인증 완료 후)
const PROGRESS_STEPS: Step[] = ["password", "bizinfo", "bizreg", "store", "name", "phone", "sms"];
...
function Btn({ children, onClick, disabled, loading }: { children: React.ReactNode; onClick: () => void; disabled?: boolean; loading?: boolean }) {
  return (
    <button onClick={onClick} disabled={disabled || loading}
      style={{ marginTop: 24, width: "100%", height: 54, border: "none", borderRadius: 14, background: "var(--p)", color: "#fff", fontWeight: 800, fontSize: 16, cursor: disabled || loading ? "not-allowed" : "pointer", opacity: disabled || loading ? 0.4 : 1, display: "flex", alignItems: "center", justifyContent: "center", gap: 8, transition: "opacity .15s" }}>
      {loading && <Spin />}{children}
    </button>
  );
}
function Spin() {
  return <div style={{ width: 16, height: 16, border: "2.5px solid rgba(255,255,255,.35)", borderTopColor: "#fff", borderRadius: "50%", animation: "spin .7s linear infinite" }} />;
}
...
function PwCheck({ pw }: { pw: string }) {
  const items = [
    { l: "8자 이상", ok: pw.length >= 8 },
    { l: "대문자 포함", ok: /[A-Z]/.test(pw) },
    { l: "숫자 포함", ok: /[0-9]/.test(pw) },
    { l: "특수문자 포함", ok: /[!@#$%^&*(),.?":{}|<>_\-]/.test(pw) },
  ];
  return (
    <div style={{ marginTop: 10, display: "flex", flexWrap: "wrap", gap: "5px 12px" }}>
      {items.map(x => <span key={x.l} style={{ fontWeight: 600, fontSize: 12, color: x.ok ? "#16a34a" : "#bbb", display: "flex", alignItems: "center", gap: 4 }}><span>{x.ok ? "✓" : "○"}</span>{x.l}</span>)}
    </div>
  );
}
...
function fmt폰(v: string) {
  const d = v.replace(/\D/g, "").slice(0, 11);
  if (d.length <= 3) return d;
  if (d.length <= 7) return `${d.slice(0,3)}-${d.slice(3)}`;
  return `${d.slice(0,3)}-${d.slice(3,7)}-${d.slice(7)}`;
}
function fmt사업자(v: string) {
...
function OwnerAuthInner() {
  ...
  const [step, setStep] = useState<Step>(
    urlStep && STEPS.includes(urlStep) ? urlStep : "email"
  );
  ...
  function back() {
    const order: Step[] = ["email", "password", "bizinfo", "bizreg", "store", "name", "phone", "sms"];
  ...
  async function sendEmailLink() {
    ...
    try {
      ...
      setErr((e as { message?: string }).message ?? "메일 발송 실패");
    } finally { setLoading(false); }
  }

  // 3. 비밀번호 설정 (이메일 인증 완료 후)
  async function setPassword() {
    if (!isStrong(pw)) { setErr("비밀번호 조건을 모두 충족해야 합니다."); return; }
    ...
    try {
      ...
      setErr((e as { message?: string }).message ?? "비밀번호 설정 실패");
    } finally { setLoading(false); }
  }
  ...
  async function verifyBiz() {
    ...
    try {
      ...
      if (!data.ok) throw new Error(data.error ?? "등록되지 않은 사업자번호입니다.");
      setBizVerified(true);
      setOk("사업자가 확인되었습니다!");
  ...
  async function finish() {
    ...
    try {
      ...
      if (!(await smsRes.json()).ok) throw new Error("인증번호가 올바르지 않습니다.");

      const { data: { user } } = await supabase.auth.getUser();
      if (!user) throw new Error("로그인 세션이 없습니다. 처음부터 다시 시도해주세요.");

      const { error: uErr } = await supabase.from("users").insert({
        id: user.id, email: user.email, phone: phone.replace(/\D/g, ""), name: ownerName, role: "owner",
      });
      ...
      const { data: store, error: sErr } = await supabase.from("stores").insert({
        ...
        business_type: bizType, employee_count: empCount,
        subscription_status: "trial", trial_ends_at: trialEndsAt,
      ...
      const cd = await (await fetch("/api/creem/checkout", {
  ...
  const pct = progressIdx >= 0 ? ((progressIdx + 1) / PROGRESS_STEPS.length) * 100 : 0;
  ...
  if (step === "done") return (
    <div style={{ minHeight: "100vh", display: "flex", alignItems: "center", justifyContent: "center", background: "#fff", flexDirection: "column", gap: 16 }}>
      <Spin /><p style={{ fontWeight: 600, fontSize: 15, color: "#555" }}>결제 페이지로 이동 중...</p>
    ...
    <div style={{ minHeight: "100vh", display: "flex", alignItems: "center", justifyContent: "center", background: "#fff", padding: 24 }}>
      <div style={{ maxWidth: 420, width: "100%", textAlign: "center" }}>
        ...
        <p style={{ fontWeight: 500, fontSize: 15, color: "#555", lineHeight: 1.65, marginBottom: 6 }}>
          <span style={{ color: "#6B4DF6", fontWeight: 700 }}>{email}</span>으로<br />인증 링크를 보냈습니다.
        </p>
        <p style={{ fontWeight: 500, fontSize: 13, color: "#aaa", lineHeight: 1.6, marginBottom: 32 }}>
          링크를 클릭하면 비밀번호 설정으로 이동합니다.<br />스팸 폴더도 확인해보세요.
        </p>
      ...
      <div style={{ maxWidth: 480, margin: "0 auto", padding: "0 24px", minHeight: "100svh", display: "flex", flexDirection: "column" }}>
        ...
        {progressIdx >= 0 && (
          <div style={{ marginTop: 16, display: "flex", alignItems: "center", gap: 10 }}>
            ...
            <span style={{ fontWeight: 600, fontSize: 12, color: "var(--text-sub)", flexShrink: 0 }}>{progressIdx + 1}/{PROGRESS_STEPS.length}</span>
          </div>
        )}

        <div style={{ flex: 1, paddingTop: 36, paddingBottom: 48 }}>
          ...
          {step === "email" && (
            <div>
              ...
              <p style={{ fontWeight: 500, fontSize: 14, color: "#888", marginBottom: 32, lineHeight: 1.5 }}>입력하신 이메일로 인증 링크를 보내드립니다.</p>
              <label style={{ fontWeight: 600, fontSize: 13, color: "#37383c", display: "block", marginBottom: 8 }}>이메일</label>
              <input type="email" value={email} onChange={e => setEmail(e.target.value)} placeholder="example@email.com" style={I} autoComplete="email" autoFocus onKeyDown={e => e.key === "Enter" && email.includes("@") && sendEmailLink()} />
              <ErrBox msg={err} />
              <Btn onClick={sendEmailLink} disabled={!email.includes("@")} loading={loading}>인증 메일 받기</Btn>
              <p style={{ marginTop: 20, textAlign: "center", fontWeight: 500, fontSize: 13, color: "#aaa" }}>
                이미 계정이 있으신가요?{" "}
                <button onClick={() => router.push("/login")} style={{ background: "none", border: "none", cursor: "pointer", fontWeight: 700, fontSize: 13, color: "var(--p)" }}>로그인</button>
              </p>
            </div>
          )}

          {/* ── 2. 비밀번호 설정 (이메일 인증 완료 후) ── */}
          {step === "password" && (
            <div>
              <p style={{ fontWeight: 700, fontSize: 13, color: "var(--p)", marginBottom: 8 }}>이메일 인증 완료</p>
              ...
              <div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
                <div>
                  ...
                  <input type="password" value={pw} onChange={e => setPw(e.target.value)} placeholder="비밀번호" style={I} autoComplete="new-password" autoFocus />
                  <PwCheck pw={pw} />
                </div>
                <div>
                  <label style={{ fontWeight: 600, fontSize: 13, color: "#37383c", display: "block", marginBottom: 7 }}>비밀번호 확인</label>
                  <input type="password" value={pwC} onChange={e => setPwC(e.target.value)} placeholder="비밀번호 재입력"
                    style={{ ...I, borderColor: pwC.length > 0 && pwC !== pw ? "#d32f2f" : "#e0e0e4" }}
                    autoComplete="new-password" />
                  {pwC.length > 0 && pwC !== pw && <p style={{ marginTop: 6, fontSize: 12, fontWeight: 600, color: "#d32f2f" }}>비밀번호가 일치하지 않습니다</p>}
                  {pwC.length > 0 && pwC === pw && <p style={{ marginTop: 6, fontSize: 12, fontWeight: 600, color: "#16a34a" }}>✓ 비밀번호가 일치합니다</p>}
                </div>
              </div>
              ...
              <ErrBox msg={err} />
              <Btn onClick={setPassword} disabled={!isStrong(pw) || pw !== pwC || !terms || !privacy} loading={loading}>다음</Btn>
            </div>
          )}
          ...
          {step === "bizinfo" && (
            <div>
              ...
              <p style={{ fontWeight: 500, fontSize: 14, color: "#888", marginBottom: 24 }}>서비스 최적화에 활용됩니다.</p>
              ...
              <div style={{ marginTop: 24 }}>
                ...
                {["1~3명","4~9명","10~19명","20명 이상"].map(v => (
                  <button key={v} onClick={() => setEmpCount(v)} style={{ width: "100%", display: "flex", alignItems: "center", gap: 12, padding: "13px 16px", marginBottom: 8, border: `2px solid ${empCount === v ? "var(--p)" : "#e8e8ea"}`, borderRadius: 14, background: empCount === v ? "var(--p-softer)" : "#fff", cursor: "pointer", fontWeight: 600, fontSize: 14, color: empCount === v ? "var(--p-tint)" : "#1a1a1a", textAlign: "left" }}>
          ...
          {step === "bizreg" && (
            <div>
              ...
              <p style={{ fontWeight: 500, fontSize: 14, color: "#888", marginBottom: 28 }}>번호 확인 후 가게 등록이 진행됩니다.</p>
              <label style={{ fontWeight: 600, fontSize: 13, color: "#37383c", display: "block", marginBottom: 8 }}>사업자등록번호</label>
              <div style={{ display: "flex", gap: 10 }}>
                <input type="text" value={bizReg} onChange={e => { setBizReg(fmt사업자(e.target.value)); setBizVerified(false); setOk(""); }} placeholder="000-00-00000" readOnly={bizVerified} autoFocus
          ...
          {step === "name" && (
            <div>
              ...
              <p style={{ fontWeight: 500, fontSize: 14, color: "#888", marginBottom: 28 }}>사장님 계정에 표시됩니다.</p>
          ...
          {step === "phone" && (
            <div>
              ...
              <p style={{ fontWeight: 500, fontSize: 14, color: "#888", marginBottom: 28 }}>직원들이 연락하는 번호입니다.</p>
              <label style={{ fontWeight: 600, fontSize: 13, color: "#37383c", display: "block", marginBottom: 8 }}>핸드폰 번호</label>
              <input type="tel" value={phone} onChange={e => setPhone(fmt폰(e.target.value))} placeholder="010-0000-0000" autoFocus style={{ ...I, letterSpacing: "0.04em" }} />
              <ErrBox msg={err} />
          ...
          {step === "sms" && (
            <div>
              ...
              <p style={{ fontWeight: 500, fontSize: 14, color: "#888", marginBottom: 28 }}>
                <span style={{ color: "var(--p)", fontWeight: 700 }}>{phone}</span>으로 보냈습니다.
              </p>
  ...
  return (
    <Suspense fallback={
      <div style={{ display: "flex", alignItems: "center", justifyContent: "center", height: "100vh" }}>
        <div style={{ width: 28, height: 28, border: "3px solid #6B4DF6", borderTopColor: "transparent", borderRadius: "50%", animation: "spin .8s linear infinite" }} />
        <style>{`@keyframes spin{to{transform:rotate(360deg)}}`}</style>
      </div>
    }>
      <OwnerAuthInner />
