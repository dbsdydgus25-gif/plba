...
const REVIEWS = [
  ...
  { quote: "노쇼 걱정 없이 마케팅할 수 있어서 너무 좋아요. 진짜 방문한 손님한테만 돈이 나가니까요.", name: "최○○ 사장님", role: "강남 헤어살롱", stars: 5 },
];
        ...
        <div className="flex gap-4" style={{ width: "max-content" }}>
          {REVIEWS.map((r, i) => (
            <motion.div
              ...
              className="w-[280px] bg-[#161616] border border-white/5 rounded-[24px] p-6 flex-shrink-0"
            >
              <div className="flex gap-0.5 mb-4">
                {[...Array(r.stars)].map((_, j) => <span key={j} className="text-yellow-400 text-[13px]">★</span>)}
              </div>
              <p className="text-[13px] text-[#D1D6DC] leading-[1.7] mb-5">&quot;{r.quote}&quot;</p>
              <div className="border-t border-white/5 pt-4">
                <p className="text-white font-bold text-[13px]">{r.name}</p>
                <p className="text-[#4E5968] text-[12px]">{r.role}</p>
              </div>
            </motion.div>
          ))}
        </div>
