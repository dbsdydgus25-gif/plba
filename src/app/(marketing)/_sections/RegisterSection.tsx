...
export default function RegisterSection() {
  ...
  return (
    <section id="register" className="py-20 px-5 bg-white border-t border-gray-100">
      <div className="max-w-lg mx-auto">
        <motion.div initial={{ opacity: 0, y: 24 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }}>
          <div className="mb-10">
            <span className="inline-block text-[11px] font-black tracking-[0.3em] text-brand uppercase bg-brand/10 border border-brand/20 px-4 py-1.5 rounded-full mb-5">사전 등록</span>
            <h2 className="text-[28px] sm:text-[36px] font-extrabold leading-[1.25] text-[#191F28]">지금 바로<br />선착순 혜택을 잡으세요</h2>
            <p className="mt-3 text-[14px] text-[#4E5968] leading-[1.7]">
              초기 등록 사장님 100분께<br />
          ...
          <form onSubmit={handleRegister} className="space-y-4">
            <div>
              <label className="block text-[13px] font-bold text-[#4E5968] mb-2">상호명</label>
              <input type="text" required value={storeName} onChange={e => setStoreName(e.target.value)}
                placeholder="예: 플바식당 홍대점"
                className="w-full px-5 py-4 bg-[#F9FAFB] border border-[#E5E8EB] rounded-2xl focus:outline-none focus:ring-2 focus:ring-brand/30 focus:border-brand text-[#191F28] text-[15px] placeholder:text-[#C4C9CF] transition-all" />
            </div>
            <div>
              <label className="block text-[13px] font-bold text-[#4E5968] mb-2">사장님 연락처</label>
              <input type="tel" required value={contact} onChange={e => setContact(e.target.value)}
                placeholder="010-0000-0000"
                className="w-full px-5 py-4 bg-[#F9FAFB] border border-[#E5E8EB] rounded-2xl focus:outline-none focus:ring-2 focus:ring-brand/30 focus:border-brand text-[#191F28] text-[15px] placeholder:text-[#C4C9CF] transition-all" />
            </div>
            <div>
              <label className="block text-[13px] font-bold text-[#4E5968] mb-2">가게 업종</label>
              <div className="relative">
                <select required value={category} onChange={e => setCategory(e.target.value)}
                  className="w-full px-5 py-4 bg-[#F9FAFB] border border-[#E5E8EB] rounded-2xl focus:outline-none focus:ring-2 focus:ring-brand/30 focus:border-brand text-[#191F28] text-[15px] appearance-none cursor-pointer transition-all">
